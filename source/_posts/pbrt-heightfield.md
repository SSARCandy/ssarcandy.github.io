...
pbrt 中的各種 shape，
有些是會先轉成 triangleMesh(對三角形求交點應該是圖學中最基礎的了，如果原本物件太複雜通常就會先把它拆成三角形組合再來算)；
而有些是有實作對 shape 交點測試的。由於把物件轉成 triangleMesh 其實就硬是多一個步驟了啊，如果可以與 shape 直接求交點，那速度上當然會大躍進~
...
那我就平均一下六個法向量來當作真正的法向量
$\underset{Normalize(}{ }\underset{MTL}{\rightarrow}  \underset{\times}{ } \underset{ML}{\rightarrow} \underset{+}{ } \underset{ML}{\rightarrow}  \underset{\times}{ } \underset{MB}{\rightarrow} \underset{+}{ } \underset{MB}{\rightarrow}  \underset{\times}{ } \underset{MBR}{\rightarrow} \underset{+}{ } \underset{MBR}{\rightarrow}  \underset{\times}{ } \underset{MR}{\rightarrow} \underset{+}{ } \underset{MR}{\rightarrow}  \underset{\times}{ } \underset{MT}{\rightarrow} \underset{+}{ } \underset{MT}{\rightarrow}  \underset{\times}{ } \underset{MTL}{\rightarrow} \underset{)}{ }$
...
稍微看看別的 shape 的交點測試，其實有很多時候會先跟 Bbox(Bounding Box) 做測試，因為與Bbox交點是容易很多的，如果與 Bbox 無交點就也不用繼續做下去了。
...
經過幾次最佳化後，成功壓低執行時間，效能比較如下:
  {% fancybox /img/2016-10-10/5.png 效能比較。 %}
