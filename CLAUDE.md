...
CI runs these four steps **individually and in order** (not `npm run build`), because `build:pageview` needs a GA credential injected between steps:
...
3. `build:vite` — bundles JS → `themes/ssarcandy/source/js/*.bundle.js`
4. `build:hexo` — `hexo generate` assembles everything into `public/`

`build:hexo` must run **last**: Hexo copies `themes/ssarcandy/source/` (including the Vite bundles) and the data JSON into `public/`. If you change JS, re-run `build:vite` before `build:hexo` or the site serves a stale bundle.
...
- `project_image_aspect(name, ext)` ← reads a local `source/img/projects/*` image's intrinsic dimensions (via `image-size`) as a CSS `aspect-ratio`, so the `/projects` Masonry grid reserves each card's image height and lays out with no reflow.

`scripts/project-lqip.js` registers `project_lqip(name, ext)` ← a tiny inlined blurred base64 placeholder (blur-up) for each project image, generated by `sharp` in a `before_generate` filter (so it runs inside `hexo generate`, no extra build step) and cached by mtime. `sharp` is optional: if its native binary is missing the placeholder is skipped, never failing the build.

All data files degrade gracefully to empty/zero when missing, so a local build never fails for lacking credentials or network. `scripts/related-posts.js` registers the related-posts logic.
...
- Search is client-side (`themes/ssarcandy/js/Search.js`): fetches `/content.json` (generated by `hexo-generator-json-content`) and regex-matches title/tags/text. `lunr` is used separately in `ProjectTags.js` for the `/projects` tag filter.
...
- **Secrets:** the GA service-account key `hexo-pv-c7938b2e210b.json` is gitignored and injected by CI from a secret; `pageview.json` and `source/flickr_photos.json` are gitignored build artifacts. Keep them out of commits.
