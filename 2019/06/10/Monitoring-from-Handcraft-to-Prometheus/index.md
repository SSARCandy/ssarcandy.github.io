...
![pm2 monitor screenshot](https://ssarcandy.tw/img/2019-06-11/01.png) 
...
![](https://ssarcandy.tw/img/2019-06-11/02.png)

老實說雖然簡陋，但完全可以一目瞭然各台機器狀況，可以稱做一個不錯的監控系統了，那時候我也基於這個網頁再實作額外的自動警報系統，當記憶體或 CPU 用量過高時，發警訊到我的 Facebook Messanger 群組。順便呢也寫了一篇文章記錄： [用 Facebook 聊天機器人當通知系統](https://ssarcandy.tw/2016/11/17/cml-fb-bot/ "用 Facebook 聊天機器人當通知系統")

從那時候之後我就秉持著要啥自己幹的原則，又利用類似 Pull Based 的方式由一台機器整合的作法，實作出另一套專門監控多台 GPU 伺服器的 [Multi-server GPU status monitor](https://ssarcandy.tw/2017/08/25/multi-server-gpu-status-monitor/ "Multi-server GPU status monitor")，實驗室大家看起來也是挺喜歡這個的 (還曾被教授關注表示讚 🥰)，至今仍在運作也令我相當開心～

![Multi-server GPU status monitor 暨用量檢舉系統一隅](https://ssarcandy.tw/img/2019-06-11/03.png) 
...
![示意圖。任何狀態: 機器死掉、閒置、目前使用狀況都顯示在同一個畫面上。](https://ssarcandy.tw/img/2019-06-11/04.png) 
